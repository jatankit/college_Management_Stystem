...
def student_session(username):
    while 1:
        ...
        if user_option == "1":
            ...
            records = command_handler.fetchall()
            for record in records:
                with open("C:/python/register.txt", "w") as f:
                    f.write(str(records)+"\n")
                f.close()
...
def auth_student():
    ...
    command_handler.execute(
        "SELECT username FROM users WHERE username = %s AND password = %s AND privilege = %s", query_vals)
...
def auth_teacher():
    ...
    query_vals = (username, password)
    command_handler.execute(
        "SELECT * FROM users WHERE username = %s AND password = %s AND privilege = 'teacher'", query_vals)
    if command_handler.rowcount <= 0:
